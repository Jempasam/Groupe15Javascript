...
export class LvlTest {
    ...
    const wall = new Wall("Wall1",-21.5, -1, -7.5, 5, 1, 20,this.scene);
    listes[2].push(wall);
    //créer un sol en hauteur
    const ground2 = new Ground("Ground2",-9, 0.5, -5, 10, 2, 5,this.scene);
    listes[1].push(ground2);
    //créer une rampe en tournant le sol
    const ground3 = new Ground("Ground3",-2, 2, -5, 10, 1, 5,this.scene);
    ground3.mesh.rotation.z = -Math.PI/6;
    ...
    //créer des monstres
    const monster = new Monster("Monster1",-20, 0, 0, 1, 1, 1, player.playerSpeed*1.2,this.scene);
    listes[0].push(monster);
    const monster2 = new Monster("Monster2",-23, 0, -5, 1, 1, 1, player.playerSpeed,this.scene);
